...
const fileUpload = (req, res = response) => {
  ...
  if (!tiposValidos.includes(tipo)) {
    return res.status(400).json({
      ok: false,
      msg: "el tipo no es hospitales, medicos o usuarios",
    });
  }

  // al pasar la ruta por el middleware puedo obtener el req.files con los archivos
  ...
  if (!extensionesValidas.includes(extensionArchivo)) {
    return res.status(400).json({
      ok: false,
      msg: "El archivo no tiene una extension valida",
    });
  }
